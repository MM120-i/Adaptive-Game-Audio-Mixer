...
MainComponent::MainComponent (AppSettings& appSettings,
      ...
      logger (appLogger)
{
    ...
    saveSettingsButton.onClick = [this] { saveSettingsFromUi(); };
    addAndMakeVisible (saveSettingsButton);

    reloadSettingsButton.onClick = [this] { reloadSettingsIntoUi(); };
...
void MainComponent::refreshSettingsSummary() {
    settingsSummaryLabel.setText ("Config: " + settingsStore.getConfigFile().getFullPathName()
                                      + " | Verbose diagnostics: "
                                      + (settings.verboseDiagnostics ? "on" : "off"),
